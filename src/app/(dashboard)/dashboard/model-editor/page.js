          ...
          {models.map((model) => (
            <Card key={model.callName} padding="md" className="flex min-w-0 flex-col gap-3">
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                ...
                {(model.temperature !== "" && model.temperature !== null && model.temperature !== undefined) && (
                  <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-500 font-medium">
                    temp {model.temperature}
                  </span>
                )}
                {(model.topP !== "" && model.topP !== null && model.topP !== undefined) && (
                  <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-500 font-medium">
                    top-p {model.topP}
                  </span>
                )}
...
function StudioFormModal({
  ...
  const [temperature, setTemperature] = useState(
    editing?.temperature === "" || editing?.temperature === null || editing?.temperature === undefined
      ? "" : String(editing.temperature)
  );
  const [topP, setTopP] = useState(
    editing?.topP === "" || editing?.topP === null || editing?.topP === undefined
      ? "" : String(editing.topP)
  );
  const [maxTokens, setMaxTokens] = useState(
    editing?.maxTokens ? String(editing.maxTokens) : ""
  );
  const [showAdvanced, setShowAdvanced] = useState(
    Boolean(
      (editing?.temperature !== "" && editing?.temperature !== null && editing?.temperature !== undefined) ||
      (editing?.topP !== "" && editing?.topP !== null && editing?.topP !== undefined) ||
      (editing?.maxTokens ? Number(editing.maxTokens) > 0 : false)
    )
  );
    ...
    try {
      const res = await fetch("/api/model-editor", {
        ...
        body: JSON.stringify({
          ...
          maxTokens: maxTokens ? Number(maxTokens) : 0,
        }),
      });
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            >
              <span className="material-symbols-outlined text-[16px] transition-transform" style={{ transform: showAdvanced ? "rotate(90deg)" : "rotate(0deg)" }}>
                chevron_right
              </span>
              Advanced <span className="font-normal">(optional — lock temperature, top-p, max tokens)</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-text-main mb-1">
                Temperature <span className="text-text-muted font-normal">(lock)</span>
              </label>
              <Input
                type="number"
                min="0"
            ...
            <div>
              <label className="block text-xs font-medium text-text-main mb-1">
                Top-P <span className="text-text-muted font-normal">(lock)</span>
              </label>
              <Input
                type="number"
                min="0"
            ...
            <div>
              <label className="block text-xs font-medium text-text-main mb-1">
                Max tokens <span className="text-text-muted font-normal">(lock)</span>
              </label>
              <Input
                type="number"
                min="0"
          ...
          <p className="text-[11px] text-text-muted -mt-2">
            Locked params override whatever the client sends. Leave empty so callers can set their own.
          </p>
          </>
          )}
