...
async function handleSingleModelChat(body, modelStr, clientRawRequest = null, request = null, apiKey = null) {
 ...
 try {
  ...
  if (override) {
   ...
   // Locked sampling params: studio values win, client values are dropped.
   // Empty string / missing = not locked, client value passes through.
   if (override.temperature !== "" && override.temperature !== null && override.temperature !== undefined && !Number.isNaN(Number(override.temperature))) {
    body.temperature = Number(override.temperature);
   }
   if (override.topP !== "" && override.topP !== null && override.topP !== undefined && !Number.isNaN(Number(override.topP))) {
    body.top_p = Number(override.topP);
   }
   if (override.maxTokens && Number(override.maxTokens) > 0) {
    body.max_tokens = Number(override.maxTokens);
   }
   if (studio) {
